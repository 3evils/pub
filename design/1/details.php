...
//Imdb Movie URL
        ...
        $title = strip_tags($html->find('div[class="title_wrapper"] h1', 0)->innertext);
        $year = strip_tags($html->find('span[id="titleYear"]', 0)->innertext);
        $title = str_replace(''.$year.'','', $title);
        $poster = $html->find('div[class="poster"] img', 0)->src;
        $cast = $html->find('class="inline">Stars: h4');
        for($i = 0; $i < sizeof($cast); $i++)
        {
            $single_cast[] = strip_tags($cast[$i]->innertext);
        }
        $summary = $html->find('div[class="summary_text"]',0)->innertext;
        $description = $html->find('div[itemprop="description"] p',0)->innertext;
//
        $rating = $html->find('span[itemprop="ratingValue"]', 0)->innertext;
        $genre = $html->find('div[itemprop="genre"] a');
        for($g = 0; $g < sizeof($genre); $g++)
        {
            $single_genre[] = strip_tags($genre[$g]->innertext);
        }
        $single_genre = implode(',', $single_genre);
        $single_cast = implode('',$single_cast);
